...
function cleanup_hass_data() {
    rm -rf /workdir/hassio/{apparmor,backup,config.json,dns,dns.json,homeassistant,homeassistant.json,ingress.json,share,ssl,tmp,updater.json}
    rm -rf /workdir/hassio/addons/{core,data,git}
}
...
function run_supervisor() {
    docker run --rm --privileged \
        ...
        -v /run/dbus:/run/dbus \
        -v "/workdir/hassio":/data \
        -v /etc/machine-id:/etc/machine-id:ro \
        -e SUPERVISOR_SHARE="/workdir/hassio" \
